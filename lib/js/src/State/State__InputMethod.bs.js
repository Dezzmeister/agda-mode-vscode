...
async function handle(state, output) {
  ...
  IM$AgdaModeVscode.lockEditor(state.editorIM);
  await Util$AgdaModeVscode.Promise_.oneByOne(output.map(function (x) {
            return function () {
              return handle$1(x);
            };
          }));
  return IM$AgdaModeVscode.unlockEditor(state.editorIM);
}
...
async function keyUpdate(state, changes) {
  while(IM$AgdaModeVscode.isEditorLocked(state.editorIM)) {
    await new Promise((function (res, _rej) {
            setTimeout(res, 0);
          }));
  };
  return await handle(state, IM$AgdaModeVscode.run(state.editorIM, Caml_option.some(state.editor), {
                  TAG: "KeyUpdate",
                  _0: changes,
...
function keyUpdate$1(state, next) {
...
function insertChar(state, $$char) {
  return keyUpdate$1(state, previous.contents + $$char);
}
...
async function keyUpdatePromptIM(state, input) {
  ...
  switch (match) {
    ...
    case "Prompt" :
        return await keyUpdate$1(state, input);
    case "None" :
        if (activationKey.endsWith(input)) {
          return await activate$1(state, input);
        } else {
...
function keyUpdateEditorIM(state, changes) {
  ...
  switch (match) {
    case "Editor" :
        return keyUpdate(state, changes);
    case "Prompt" :
    case "None" :
        return Promise.resolve();
